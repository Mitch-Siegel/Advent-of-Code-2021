...
int main()
{
	...
	// keep track of the boards in play
	std::vector<BingoBoard> boardsInPlay = boardsList;
	for (int theMove : numbersDrawn)
	{
		std::cout << "Calling " << theMove << " - "
				  << boardsInPlay.size() << " board(s) remaining" << std::endl;

		std::set<int> removalIndices; // track all indices to be deleted
		for (int i = 0; i < boardsInPlay.size(); i++)
		{
			// execute the move on each board
			int result = boardsInPlay[i].move(theMove);
			// if this board didn't win, skip
			if (!result)
				continue;

			// if we stil have boards to remove
			if (boardsInPlay.size() > 1)
			{
				// erase the board and back the iterator up by one to account for the deletion
				boardsInPlay.erase(boardsInPlay.begin() + i--);
			}
			else // otherwise this is the last remaining board and it just won, so print the score and exit
			{
				boardsInPlay[i].printBoard();
				std::cout << "Win detected - this is the last board! \nFinal score is: " << (result * theMove) << std::endl;
				exit(0);
			}
		}
	}
